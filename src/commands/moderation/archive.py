...
class ArchiveSystem(commands.Cog):
    ...
    async def export_chat(
        ...
        if limit > 2000:
            await interaction.response.send_message("❌ Ліміт не може перевищувати 2000 повідомлень за раз (захист від спам-лімітів).", ephemeral=True)
            return
            
        if from_channel.id == to_channel.id:
            await interaction.response.send_message("❌ Канали джерела і призначення не можуть збігатися.", ephemeral=True)
            return
        ...
        if not isinstance(target_channel_for_webhook, discord.TextChannel):
            await interaction.response.send_message("❌ Вебхуки підтримуються тільки у текстових каналах або гілках текстових каналів.", ephemeral=True)
            return
        ...
        if not webhook:
            try:
                ...
                await interaction.followup.send("❌ У бота немає прав керувати вебхуками в цільовому каналі.", ephemeral=True)
                return
        ...
        try:
            ...
            await interaction.followup.send("❌ У бота немає прав читати історію в початковому каналі.", ephemeral=True)
            return

        if not messagesToExport:
            await interaction.followup.send("❌ В цьому каналі немає жодного повідомлення.", ephemeral=True)
            return

        # Робимо прогрес-бар
        total = len(messagesToExport)
        progress_msg = await interaction.followup.send(f"⏳ Скопійовано: 0/{total}... (це може зайняти хвилину)", ephemeral=True, wait=True)
        ...
        for i, msg in enumerate(messagesToExport):
            ...
            if (i + 1) % 10 == 0:
                try:
                    await progress_msg.edit(content=f"⏳ Скопійовано: {i+1}/{total}...")
                except:
                    pass

        # 4. Фінал
        await progress_msg.edit(content=f"✅ Експорт завершено!\nУспішно: **{success}**\nПомилок: **{failed}**\nПеревірте канал {to_channel.mention}.")
