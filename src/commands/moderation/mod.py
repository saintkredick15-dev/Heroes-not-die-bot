"""
mod.py
Єдиний файл для moderation команд: warn, warns, warnings, purge, mute, unmute, kick, ban.
"""
...
def _build_warns_embed(
    *,
    ...
    description = "Ось активні та попередні варни у цьому сервері." if self_view else f"Користувач: {member.mention}"
    embed = discord.Embed(
        title=f"<:warning:1485598476850040843> {title}",
    ...
    for warn in warns:
        ...
        expired, status = _warn_status(ts, decay_days)
        if not expired:
            active_count += 1

        moderator_text = f"<@{moderator_id}>" if moderator_id else "Система"
        embed.add_field(
            name=f"#{case_id} — {ts_str} {status}".strip(),
            value=f"**Причина:** {reason}\n**Модератор:** {moderator_text}",
            inline=False,
        )

    footer = f"Активних варнів: {active_count} / {len(warns)} загалом"
    if decay_days > 0:
