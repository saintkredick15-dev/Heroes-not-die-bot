...
FAQ_DATA = {
    ...
    "tickets": {
        ...
        "desc": (
            "`/ticket_setup` публікує панель створення тікетів і задає категорію, staff roles та лог-канал.\n\n"
            "**Як це працює:**\n"
            "• користувач натискає кнопку на панелі\n"
            "• бот створює окремий тікет-канал\n"
            "• staff бере тікет через `Claim`\n"
            "• закриття через `Close` просить причину й відправляє `close summary` та `transcript`\n\n"
            "**Що важливо знати:**\n"
