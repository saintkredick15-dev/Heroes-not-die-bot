...
from commands.administration.economy_setup_shared import (
    ...
    normalize_daily_cooldown_seconds,
)
...
class CaptchaModal(discord.ui.Modal, title="Перевірка на людяність"):
    captcha_input = discord.ui.TextInput(
        ...
        placeholder="Літери і цифри з малюнка",
        min_length=4,
        max_length=6,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if self.captcha_input.value.strip() == self.expected_text:
            await interaction.response.edit_message(
                content=f"{E_CHECK} Капча пройдена успішно!",
                embed=None,
                view=None,
                attachments=[],
            )
        ...
        await interaction.response.edit_message(
            content=f"{E_CROSS} Неправильний текст. Спробуй ще раз пізніше.",
            embed=None,
            view=None,
            attachments=[],
        )
...
class DailyCommand(commands.Cog):
    ...
    async def execute_daily(self, interaction: discord.Interaction, eco: dict, user_data: dict, now: int, last_daily: int):
        cooldown = normalize_daily_cooldown_seconds(eco)
        ...
        embed = surface_embed(
            ...
            (
                ...
                f"**Базова нагорода:** {base_amount} {emoji}\n"
                f"**Бонус ({streak} {E_FLAME}):** +{streak_bonus} {emoji}\n\n"
                f"**Всього зараховано:** {earned_text}"
            ),
        )
        ...
        try:
            ...
            if last_daily and (now - last_daily) < cooldown:
                ...
                await interaction.response.send_message(
                    f"{E_HOURGLASS} Ти вже отримав свою щоденну нагороду! Повертайся через **{time_str}**.",
                    ephemeral=True,
                )
                return
            ...
            if eco.get("captcha_enabled", False):
                ...
                embed = surface_embed(
                    ...
                    "Розв'яжи капчу, щоб отримати щоденну нагороду.\nНатисни **Відповісти**, коли будеш готовий.",
                    tone="warning",
                )
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message(f"{E_WARN} Помилка: `{exc}`", ephemeral=True)
            else:
                await interaction.followup.send(f"{E_WARN} Помилка: `{exc}`", ephemeral=True)
